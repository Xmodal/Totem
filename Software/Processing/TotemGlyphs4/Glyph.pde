...
class Glyph
{
  ...
  // Creates a new glyph using the given generative parameters.
  Glyph(GlyphBlockFactory topFactory, GlyphBlockFactory subFactory)
  {
    // Create top-level blocks.
    topBlocks = new GlyphBlock[N_TOP_BLOCKS];
    for (int i=0; i<N_TOP_BLOCKS; i++)
      topBlocks[i] = topFactory.create();
    GlyphBlock emptyBlock = new GlyphBlock();
          ...
          if (topBlocks[i].get(j, k)) {
            if (random(1) < subdivisionProbability)
            {
              ...
              newBlockOnTheBlock = new GlyphBlock(
                ...
                currentTopBlock.getIfExists(j-1, k, prevTopBlock, nextTopBlock),
                currentTopBlock.getIfExists(j, k+1),
                currentTopBlock.getIfExists(j+1, k, prevTopBlock, nextTopBlock));
              //newBlockOnTheBlock = new GlyphBlock(
