...
class Glyph
{
  ...
  Glyph(float[] typeWeights, float[] rotationWeights, float[] translationWeights)
  {
    ...
    {
      topBlocks[i] = generateGlyphBlock(typeWeights, rotationWeights, translationWeights);

      for (int j=0; j<BLOCK_SIDE; j++) {
        for (int k=0; k<BLOCK_SIDE; k++) {
          blocks[i][j][k] = 
            // If top-level block is false at this point generate an empty sub-block.
            (topBlocks[i].get(j, k) ?
            generateGlyphBlock(typeWeights, rotationWeights, translationWeights) :
            new GlyphBlock(TYPE_VOID, ROTATION_0, TRANSLATION_TOP));
        }
      }
    }
